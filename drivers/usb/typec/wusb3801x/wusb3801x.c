...
/*extern bool get_hub_disconnect_status(void);
extern void set_hub_disconnect_status(void);
*/
...
/* add for typec_port node */
	...
	union power_supply_propval val1;
...
static DECLARE_WAIT_QUEUE_HEAD(wq_resume_ready);
static bool wusb3801_flag = true;

static int wusb3801_read_byte(struct wusb3801_chip *chip, u8 reg)
{
	int ret;
	int count = 3;
...
static int wusb3801_write_byte(struct wusb3801_chip *chip, u8 reg, u8 data)
{
	int ret = 0;
	mutex_lock(&chip->i2c_rw_lock);
...
static void wusb3801_read_current(struct wusb3801_chip *chip){
	...
	rc = wusb3801_read_byte(chip, WUSB3801_REG_STATUS);
					...
					uint8_t addr, uint8_t mask, uint8_t val)
{
	...
	rc = wusb3801_read_byte(chip, addr);
	if (!IS_ERR_VALUE_64(rc)){
		rc = wusb3801_write_byte(chip,
...
static int wusb3801_read_device_id(struct wusb3801_chip *chip)
{
	struct device *cdev = &chip->client->dev;
	int rc;
	rc = wusb3801_read_byte(chip, WUSB3801_REG_VERSION_ID);
	if (IS_ERR_VALUE_64(rc))
		return rc;
	...
	rc = wusb3801_read_byte(chip, WUSB3801_REG_TEST_01);
	if (IS_ERR_VALUE_64(rc))
		return rc;
	chip->dev_sub_id = rc & WUSB3801_VENDOR_SUB_ID_MASK;
	if (chip->chip_id == CHIP_AW)
		chip->dev_sub_id = 0x80;
	dev_err(cdev, "VendorSUBID register: 0x%02x\n", chip->dev_sub_id);
	return 0;
}

static int get_chip_id(struct wusb3801_chip *chip)
{
	struct device *cdev = &chip->client->dev;
	int rc, rc1;

	rc = wusb3801_read_byte(chip, WUSB3801_REG_TEST_01);
	rc1 = wusb3801_read_byte(chip, WUSB3801_REG_TEST_02);
	if (IS_ERR_VALUE_64(rc) || IS_ERR_VALUE_64(rc1))
		return rc;
	dev_info(cdev, "chip_id reg0x7: 0x%02x, chip_id reg0x8: 0x%02x\n", rc, rc1);

	if (rc == 0x4F && rc1 == 0x34)
		chip->chip_id = CHIP_AW;
	else
		chip->chip_id = CHIP_WUSB;
...
static int wusb3801_update_status(struct wusb3801_chip *chip)
{
	...
	rc = wusb3801_read_byte(chip, WUSB3801_REG_CONTROL0);
	if (IS_ERR_VALUE_64(rc)) {
...
static int wusb3801_set_mode(struct wusb3801_chip *chip, uint8_t mode)
{
	...
	if (mode != chip->mode) {
		...
		rc = wusb3801_write_byte(chip, WUSB3801_REG_CONTROL0, rc);
		...
		rc = wusb3801_read_byte(chip, WUSB3801_REG_INTERRUPT);
		if (IS_ERR_VALUE_64(rc)) {
				...
				char *buf)
{
	...
	for (i = WUSB3801_REG_VERSION_ID ; i <= WUSB3801_REG_TEST_12; i++) {
		rc = wusb3801_read_byte(chip, (uint8_t)i);
		if (IS_ERR_VALUE_64(rc)) {
...
static int typec_partner_register(struct wusb3801_chip *chip)
{
	int rc = 0;
	if (IS_ERR_OR_NULL(chip->typec_port)) {
		pr_err("failed to register typec_partner_register rc=%d\n", rc);
		return -EINVAL;
	}

	if(!chip->typec_partner) {
		if (chip->type == WUSB3801_TYPE_AUD_ACC) {
			chip->typec_partner_desc.accessory = TYPEC_ACCESSORY_AUDIO;
		} else {
		...
		if (IS_ERR(chip->typec_partner)) {
			rc = PTR_ERR(chip->typec_partner);
			pr_err("failed to register typec_partner rc=%d\n", rc);
		}
	}

	if (chip->type == WUSB3801_TYPE_SNK){
...
static void typec_partner_unregister(struct wusb3801_chip *chip)
{
	if (IS_ERR_OR_NULL(chip->typec_port))
        	return;

	if (!IS_ERR_OR_NULL(chip->typec_partner)) {
        	pr_err("Un-registering typeC partner\n");
...
static int reset_patch(struct wusb3801_chip *chip)
{
	...
	struct device *cdev = &chip->client->dev;
	//reg10 write 0x55
	rc = wusb3801_write_byte(chip, WUSB3801_REG_TEST_04, 0x55);
	if (IS_ERR_VALUE_64(rc)) {
	...
	msleep(25);
	//reg10 write 0x00
	rc = wusb3801_write_byte(chip, WUSB3801_REG_TEST_04, 0x00);
	if (IS_ERR_VALUE_64(rc)) {
...
{
	struct device *cdev = &chip->client->dev;
	union power_supply_propval val_nothing = {0};
#ifdef __RESET_PATCH__
	int ret;
#endif /* __RESET_PATCH__ */
...
{
	int rc;
	int rc_reg_08,i=0,ret=0; //malei
	struct device *cdev = &chip->client->dev;
	dev_err(cdev, "%s \n",__func__);

	msleep(50);
	for (i = 0; i < 60; i++) {
		ret = wusb3801_write_byte(chip, WUSB3801_REG_TEST_02, 0x80);
		if (IS_ERR_VALUE_64(ret)) {
			dev_err(cdev, "%s: WUSB3801_REG_TEST_02 failed to write 0x80, ret = %d\n", __func__, ret);
		}
		rc_reg_08 = wusb3801_read_byte(chip, WUSB3801_REG_TEST_02);
		if ((IS_ERR_VALUE_64(rc_reg_08)) ||(rc_reg_08 != 0x80)) {
			dev_err(cdev, "%s: WUSB3801_REG_TEST_02 failed to read, rc_reg_08 = %d\n", __func__, rc_reg_08);
		}
		ret = wusb3801_write_byte(chip, WUSB3801_REG_TEST_09, 0x02);
		if (IS_ERR_VALUE_64(ret)) {
			dev_err(cdev, "%s: WUSB3801_REG_TEST_09 failed to write 0x02, ret = %d\n", __func__, ret);
		}
		rc_reg_08 = wusb3801_read_byte(chip, WUSB3801_REG_TEST_09);
		if ((IS_ERR_VALUE_64(rc_reg_08)) ||(rc_reg_08 != 0x02)) {
			dev_err(cdev, "%s: WUSB3801_REG_TEST_09 failed to read, rc_reg_09 = %d\n", __func__, rc_reg_08);
		}
		rc = wusb3801_read_byte(chip, WUSB3801_REG_TEST0);
		if (IS_ERR_VALUE_64(rc)) {
			dev_err(cdev, "%s: WUSB3801_REG_TEST0 failed to read, rc_reg_06 = %d\n", __func__, rc);
		} else if (rc == 0xB2) {
			break;
		}
		msleep(10);
	}
	dev_err(cdev, "%s: WUSB3801 TEST rc = %d\n", __func__, rc);
	for (i = 0; i < 5; i++) {
		ret = wusb3801_write_byte(chip, WUSB3801_REG_TEST_02, 0);
		if (IS_ERR_VALUE_64(ret)) {
			dev_err(cdev, "%s: WUSB3801_REG_TEST_02 failed to write 0, ret = %d\n", __func__, ret);
		}
		rc_reg_08 = wusb3801_read_byte(chip, WUSB3801_REG_TEST_02);
		if ((IS_ERR_VALUE_64(rc_reg_08)) ||(rc_reg_08 != 0)) {
			dev_err(cdev, "%s: WUSB3801_REG_TEST_02 failed to read, rc_reg_08 = %d\n", __func__, rc_reg_08);
		}
		ret = wusb3801_write_byte(chip, WUSB3801_REG_TEST_09, 0);
		if (IS_ERR_VALUE_64(ret)) {
			dev_err(cdev, "%s: WUSB3801_REG_TEST_09 failed to write 0, ret = %d\n", __func__, ret);
		}
		rc_reg_08 = wusb3801_read_byte(chip, WUSB3801_REG_TEST_09);
		if ((IS_ERR_VALUE_64(rc_reg_08)) ||(rc_reg_08 != 0)) {
			dev_err(cdev, "%s: WUSB3801_REG_TEST_09 failed to read, rc_reg_09 = %d\n", __func__, rc_reg_08);
	...
	ret = wusb3801_write_byte(chip, WUSB3801_REG_CONTROL1, 1);
	if (IS_ERR_VALUE_64(ret)) {
		dev_err(cdev, "%s: WUSB3801_REG_CONTROL1 failed to write 1, ret = %d\n", __func__, ret);
	}
	msleep(100);
	rc = (rc==0xB2) ? 0xF2 : 0xB2;
...
static unsigned wusb3801_is_waiting_for_vbus(struct wusb3801_chip *chip)
{
  struct device *cdev = &chip->client->dev;
	int rc;
	rc = wusb3801_read_byte(chip, WUSB3801_REG_TEST_0A);
	if (IS_ERR_VALUE_64(rc)) {
...
static int get_cc_status(struct wusb3801_chip *chip)
{
	...
	if (IS_ERR_VALUE_64(rc)) {
		dev_err(cdev, "%s cannot read WUSB3801_REG_STATUS:rc = %d\n", __func__, rc);
		rc = 0xFF;
	}
	rc  =  BITS_GET(rc, WUSB3801_CC_STS_MASK);
	dev_info(cdev, "rc1 = %d\n", rc);
	...
	dev_info(cdev, "%s get typec cc_sts = %d\n", __func__, rc);
	return rc;
}
...
static void wusb3801_attach(struct wusb3801_chip *chip)
{
	...
	union power_supply_propval val_src = {6};
	union power_supply_propval val_snk = {1};
	union power_supply_propval val_aud = {4};
	uint8_t status, type;
	...
	if (IS_ERR_VALUE_64(rc)) {
		...
		if(chip->dev_sub_id != 0xA0){
			if(chip->chip_id == CHIP_WUSB && chip->cc_test_flag == 0 &&  BITS_GET(rc, WUSB3801_CC_STS_MASK) == 0 ){
				//chip->reg4_val = rc;
				chip->cc_sts = test_cc_patch(chip);
				chip->cc_test_flag = 1;
				dev_err(cdev, "%s: cc_sts[0x%02x]\n", __func__, chip->cc_sts);
				return;
			} else if (chip->cc_test_flag == 0 ){
				chip->cc_sts = get_cc_status(chip);
				dev_err(cdev, "%s: AW_cc_sts[0x%02x]\n", __func__, chip->cc_sts);
			}
			if(chip->chip_id == CHIP_WUSB && chip->cc_test_flag == 1 ){
				chip->cc_test_flag = 0;
...
#endif	/* __TEST_CC_PATCH__ */
	chip->val.intval = chip->cc_sts;
	pr_info("wusb3801 get_cc_status = %d\n", chip->val.intval);
	status = (rc & WUSB3801_ATTACH) ? true : false;
	type = status ? \
			rc & WUSB3801_TYPE_MASK : WUSB3801_TYPE_INVALID;
	dev_info(cdev, "sts[0x%02x], type[0x%02x]\n", status, type);

	if ((chip->chip_id != CHIP_AW) && (chip->state != WUSB3801_STATE_ERROR_RECOVERY)) {
		rc = wusb3801_set_chip_state(chip,
				WUSB3801_STATE_ERROR_RECOVERY);
		if (IS_ERR_VALUE_64(rc))
	...
	case WUSB3801_TYPE_SRC:
		...
		break;
	case WUSB3801_TYPE_SNK:
		chip->val.intval = get_cc_status(chip);
	...
	chip->attached = true;
	typec_partner_register(chip);
}
static irqreturn_t wusb3801_work_handler(int irq, void *data)
{
	...
	if (wusb3801_flag == false) {
		wait_event_interruptible_timeout(wq_resume_ready,
			wusb3801_flag, msecs_to_jiffies(1000));
		dev_err(cdev, "%s: wait_event_interruptible_timeout 1000ms\n", __func__);
	}

	if (wusb3801_flag == false) {
		dev_err(cdev, "%s: The device not resume 1000ms\n", __func__);
		goto work_unlock;
	}
	...
	dev_info(cdev,"%s WUSB3801_REG_STATUS : 0x%02x, int_sts[0x%02x]\n", __func__, rc, int_sts);
	...
	if (int_sts & WUSB3801_INT_ATTACH){
		if(chip->attached){
			if (1) {
				//if(get_hub_disconnect_status()) {
				dev_info(cdev, "wusb3801 it's otg plug out\n");
				wusb3801_detach(chip);
				//set_hub_disconnect_status();
			} else {
...
static int wusb3801_iio_read_raw(struct iio_dev *indio_dev,
		struct iio_chan_spec const *chan, int *val1,
		int *val2, long mask)
{
	struct wusb3801_chip *chip = iio_priv(indio_dev);
	int rc = 0;

	*val1 = 0;

	switch (chan->channel) {
	case PSY_IIO_CC_CHIP_ID:
		*val1 = chip->chip_id;
		break;
	case PSY_IIO_TYPEC_CC_ORIENTATION:
		*val1 = chip->val.intval;
		break;
	case PSY_IIO_TYPEC_MODE:
		*val1 = chip->val1.intval;
		break;
	default:
		pr_info("Unsupported wusb3801 IIO chan %d\n", chan->channel);
		rc = -EINVAL;
		break;
	}
...
static int wusb3801_iio_write_raw(struct iio_dev *indio_dev,
		struct iio_chan_spec const *chan, int val1,
		int val2, long mask)
{
	//struct wusb3801_chip *chip = iio_priv(indio_dev);
	...
	case PSY_IIO_TYPEC_MODE:
		break;
	default:
		pr_err("Unsupported wusb3801 IIO chan %d\n", chan->channel);
		rc = -EINVAL;
		break;
	}
	if (rc < 0)
...
static int wusb3801_iio_of_xlate(struct iio_dev *indio_dev,
				const struct of_phandle_args *iiospec)
{
	...
	for (i = 0; i < ARRAY_SIZE(wusb3801_iio_psy_channels);
					i++, iio_chan++)
...
static const struct iio_info wusb3801_iio_info = {
	.read_raw	= wusb3801_iio_read_raw,
	.write_raw	= wusb3801_iio_write_raw,
	.of_xlate	= wusb3801_iio_of_xlate,
};

static int wusb3801_init_iio_psy(struct wusb3801_chip *chip)
{
	struct iio_dev *indio_dev = chip->indio_dev;
	struct iio_chan_spec *chan;
	int num_iio_channels = ARRAY_SIZE(wusb3801_iio_psy_channels);
	int rc, i;

	pr_err("wusb3801_init_iio_psy start\n");
	...
	for (i = 0; i < num_iio_channels; i++) {
		...
		chan->address = i;
		chan->channel = wusb3801_iio_psy_channels[i].channel_num;
		chan->type = wusb3801_iio_psy_channels[i].type;
		chan->datasheet_name =
			wusb3801_iio_psy_channels[i].datasheet_name;
		chan->extend_name =
			wusb3801_iio_psy_channels[i].datasheet_name;
		chan->info_mask_separate =
			wusb3801_iio_psy_channels[i].info_mask;
	}

	rc = devm_iio_device_register(chip->dev, indio_dev);
	if (rc)
		pr_err("Failed to register wusb3801 IIO device, rc=%d\n", rc);

	pr_err("wusb3801 IIO device, rc=%d\n", rc);
	return rc;
}
			...
			const struct i2c_device_id *id)
{
	...
	pr_info("wusb3801_really start, cnt=%d!\n", probe_cnt);
	...
	if (!indio_dev){
		dev_err(cdev, "Failed to allocate memory\n");
		return -ENOMEM;
	}
	...
	if (ret1 == CHIP_AW)
	{
		dev_err(cdev, "cc logic: aw35616 -> soft reset\n");
		wusb3801_write_byte(chip, 0x06, 0x02); // for ic soft reset clear reg
	}
	else if (ret1 == CHIP_WUSB)
		dev_err(cdev, "cc logic: wusb3801\n");
	else
		dev_err(cdev, "cc logic: unknown\n");
	...
	chip->typec_port = typec_register_port(cdev, &chip->typec_caps);

	if (IS_ERR_OR_NULL(chip->typec_port)) {
        	ret = PTR_ERR(chip->typec_port);
        	pr_err("failed to register typec_port ret=%d\n", ret);
        	goto err4;
	}
	...
	ret = wusb3801_init_iio_psy(chip);
	if (ret < 0) {
		pr_err("Failed to initialize wusb3801 IIO PSY, rc=%d\n", ret);
	}
	...
	ret =
	    request_threaded_irq(chip->irq_gpio, wusb3801_interrupt, wusb3801_work_handler, IRQF_TRIGGER_LOW | IRQF_ONESHOT, "wusb3801_int_irq", chip);
	if (ret != 0) {
	...
	ret = devm_request_threaded_irq(cdev, chip->irq_gpio,
				wusb3801_interrupt,
				wusb3801_work_handler,
...
#endif /*__MEDIATEK_PLATFORM__*/
	wusb3801_write_byte(chip, WUSB3801_REG_TEST_02, 0x00);
	wusb3801_write_byte(chip, WUSB3801_REG_TEST_09, 0x00);
	wusb3801_write_byte(chip, WUSB3801_REG_CONTROL1, BIT_REG_CTRL1_SM_RST);
	...
	ret = wusb3801_read_byte(chip, WUSB3801_REG_STATUS);
	if (IS_ERR_VALUE_64(ret)) {
		dev_err(cdev, "%s: failed to read reg status\n", __func__);
	}
...
{
	wusb3801_flag = true;
	wake_up_interruptible(&wq_resume_ready);
	return 0;
}
