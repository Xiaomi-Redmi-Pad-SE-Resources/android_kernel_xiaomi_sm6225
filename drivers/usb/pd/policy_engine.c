...
static ssize_t pd_vbus_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct usbpd *pd = dev_get_drvdata(dev);
...
static ssize_t pd_vbus_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int val = 0;
	if (sscanf(buf, "%d\n", &val) != 0)
	{
